;(function (root) {
    ...
    function require(id, url) {
        ...
        return data.modules[url] && data.modules[url].exports;
    }
    ...
    function execMod(id, callback, params) {
        ...
        if (!params) {
            ...
            data.modules[id].exports = data.modMap[id].callback;
        } else {
            curExecModName = id;
			
            //commonjs
            var exp = data.modMap[id].callback.apply(null, params);
            curExecModName = null;
            //amd和返回值的commonjs
        ...
        //执行回调函数
        callback(data.modules[id].exports);
    ...
    function loadMod(id, callback, option) {
        ...
        if (id === 'exports') {
            var exports = data.modules[option.baseUrl].exports = {};
            callback(exports);
            return -2;
        }
        if (id === 'module') {
            callback(data.modules[option.baseUrl]);
            return -3;
        }
        id = getDepUrl(id, option.baseUrl);
        ...
        if (!data.modMap[id]) {
            ...
            loadjs(id, function () {
                ...
                //如果define的不是函数
                if (!isFn(data.modMap[id].callback)) {
                ...
                //define的是函数
                use(data.modMap[id].deps, function () {
        ...
        //加载失败
        if (data.modMap[id].status === 'error') {
            callback();
            return 1;
        }
        //正在加载
        if (data.modMap[id].status === 'loading') {
            data.modMap[id].oncomplete.push(callback);
    ...
    function define(name, deps, callback) {
        ...
        var id = getIdUrl(name).split('?')[0];//fix 后缀

        data.modMap[id] = data.modMap[id] || {};
        data.modMap[id].deps = deps;
        data.modMap[id].callback = callback;
        data.modMap[id].status = 'loaded';
        data.modMap[id].oncomplete = data.modMap[id].oncomplete || [];
        data.modules[id] = {};
        return 0;
    }
    ...
    function debug() {
        console.log(data.modMap, data.modules);
    }
    var lodjs = {
        version: '0.1.0',
        use: use,
